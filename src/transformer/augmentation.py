...
class DataAugmenter:
    def __init__(self, noise_std=0.005, scale_range=(0.95, 1.05)):
    ...
    def mask_features(self, x, mask_prob=0.05):
        """Randomly mask out some time steps (dropout on input)."""
        mask = np.random.binomial(1, 1 - mask_prob, x.shape[0])
    ...
    def augment(self, x):
        ...
        if np.random.rand() > 0.8: # Less frequent
